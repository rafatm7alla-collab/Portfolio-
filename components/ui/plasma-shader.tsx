...
// Palette darkened for Rafat Mhalla: near-black plasma so it reads as a
// slow-moving deep-black surface rather than a grey wash.
...
// Palette: near-black with subtle warm/cool variation. Keeps the shader
// obviously moving without ever brightening past #1f1f1f.
const UNIFORMS = {
  colors: [
    [0.00, 0.00, 0.00],
    [0.55, 0.55, 0.55],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const glRaw = canvas.getContext("webgl", { antialias: false })
    if (!glRaw) return
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      bounds = c.getBoundingClientRect(); updatePointerTarget()
    }
    const onPointerLeave = () => { pointerKnown = false; targetPresence = 0; requestRender() }
    const updateLayout = () => { bounds = c.getBoundingClientRect(); resizeCanvas(); updatePointerTarget(); requestRender() }
